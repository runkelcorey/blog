...
tidal_data <- responses |>
    ...
    set_names(map(fifteen_minutes, "url")) |> # name each element according to its url
...
tidal_data |>
    ...
    labs(x = "Timestamp (August 6-7, 2025)", y = "Time-series code", fill = "% of stations\nreporting") +
    theme(legend.position = "bottom")
```
...
tidal_data |>
    ...
    ggplot() +
    aes(x = eventDate, y = id, fill = not_null) +
    geom_tile(color = "gray70") +
    scale_fill_discrete() +
    scale_x_datetime(date_labels = "%H%p", date_minor_breaks = "1 hour") +
    labs(x = "Timestamp (August 6-7, 2025)", y = "Station") +
